        ...
        {filteredNodes.map((node) => {
          ...
          return (
            ...
            >
              <div>
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <h3 className="font-display text-base text-white font-normal">{node.title}</h3>
                  </div>
                  <button
                    type="button"
                    onClick={() => onFocusNodeOnCanvas(node.id)}
                    className="flex items-center gap-1 text-xs text-rose-400 hover:text-rose-300 font-body"
                  >
                    <span>Inspect</span>
                ...
                {node.subtitle && (
                  <p className="text-[10px] text-zinc-500 font-tech mb-3">{node.subtitle}</p>
                )}

                {/* Profile */}
                {node.category === 'profile' && node.profile && (
                  <p className="text-xs text-zinc-300 font-body leading-relaxed">{node.profile.bio}</p>
                )}
                ...
                {node.category === 'skills' && node.skills && (
                  <div className="space-y-1.5 pt-1">
                    {node.skills.map((s) => (
                      <div key={s.name} className="flex items-baseline justify-between text-xs py-0.5 border-b border-white/[0.03]">
                        <span className="text-zinc-300 font-body">{s.name}</span>
                        <span className="font-tech text-[10px] text-zinc-500">{s.category}</span>
                      </div>
                    ))}
                  </div>
                )}
                  ...
                  <div className="space-y-2 pt-1">
                    {node.certificates.map((cert) => (
                      <div
                        ...
                        className="p-2.5 rounded-lg bg-white/[0.02] hover:bg-white/[0.05] border border-white/[0.04] cursor-pointer transition-colors"
                      >
                        <div className="text-xs font-body font-medium text-zinc-200">{cert.title}</div>
                ...
                {node.category === 'project' && node.project && (
                  <div className="space-y-2 pt-1">
                    <p className="text-xs text-zinc-300 font-body leading-relaxed">{node.project.description}</p>
                    <button
                      type="button"
                      onClick={() => onOpenProjectModal(node.project!)}
                      className="text-xs font-body text-rose-400 hover:text-rose-300 transition-colors pt-1 inline-block"
                    >
              ...
              <div className="mt-4 pt-3 border-t border-white/[0.04] flex items-center justify-between text-[10px] font-tech text-zinc-500">
                <span>Active Connections: {outgoingConns.length}</span>
                <span className="uppercase text-rose-500/80">{node.category}</span>
              </div>
            </div>
          );
        })}
